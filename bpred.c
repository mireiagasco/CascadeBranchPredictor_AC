...
char cascade=0;
char cascadeUtilitzarBimod;
char cascadeBimodHaFallat;
	     ...
	     struct bpred_update_t *dir_update_ptr)/* pred state pointer */
{
  ...
  {
    pred->dir_hits++;
    if(cascade && cascadeUtilitzarBimod)
    	cascadeBimodHaFallat=0;
   }
  else
  {
    pred->misses++;
    if(cascade && cascadeUtilitzarBimod )
    	cascadeBimodHaFallat=1;
  }
